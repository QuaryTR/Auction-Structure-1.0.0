...
public class Pagination {
    ...
    public void addItems(final ClickableItem clickableItem){
        this.clickableItems.add(clickableItem);
        this.updateInventory();
    }

    public void setItem(final ClickableItem clickableItem,final int slot,boolean update){
        this.clickableItems.set(slot,clickableItem);
        if  (update) this.updateInventory();
    }

    public void removeItems(final int slot,final boolean update){

        this.clickableItems.remove(slot);
        if (update) updateInventory();

        if (clickableItems.isEmpty()){
            Integer[] array = itemSlots.toArray(new Integer[0]);
            Arrays.sort(array);
            Inventory inventory = getInventory().getInventory();
            inventory.setItem(array[0],new ItemStack(Material.AIR));
        }
    }
    ...
    private boolean updateInventory() {
        ...
        for (; first < last; first++) {
            ...
            this.hInventory.setItem(this.itemSlots.get(m), clickableItem);
            m++;
        }
        return true;
    }
